...
def bbf_Traffic(p1, p2, p3, p4):
    ...
    model = Traffic_sign.LaplaceFT(p1, p2, p3, p4)
    to_device(model, device)
    history = fit_one_cycle_Bayes(20, 0.01, model, train_dl, valid_dl, opt_func=torch.optim.Adam)
    torch.save(model.state_dict(), './results/Laplace/Traffic/Traffic-{}-{}-{}-{}.pth'.format(p1, p2, p3, p4))
    # AVERAGE RUN 30 TIMES
    num = 20
    std = 0.9
    evaluated = np.zeros(num)
    for i in range(num):
        model.load_state_dict(torch.load('./results/Laplace/Traffic/Traffic-{}-{}-{}-{}.pth'.format(p1, p2, p3, p4)))
    ...
    global best_accu
    if accu > best_accu:
        torch.save(model.state_dict(), './results/Laplace/Traffic/Traffic-{}@0.9.pth'.format(accu))
        with open('./results/Laplace//Traffic/Traffic-{}@0.9.txt'.format(accu), "w") as f:
            print(model, file=f)
        ...
        fig.savefig("./results/Laplace/Traffic/best_accu_{}.png".format(accu), dpi=320, bbox_inches='tight')
        results = np.vstack((S, np.array(A)))
        np.save("./results/Laplace/Traffic/best_accu_{}.npy".format(accu), results)
...
def step_Traffic(n_iter):
    ...
    logger = JSONLogger(path="./results/Laplace/log/Traffic/logs1.json")
