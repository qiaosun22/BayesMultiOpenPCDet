...
def bbf_LeNet(p1, p2, p3, p4, p5):
    ...
    model = LeNet.LeNet_Gauss(p1, p2, p3, p4, p5)
    ...
    torch.save(model.state_dict(), './results/Gauss/LeNet/LeNet-{}-{}-{}-{}-{}.pth'.format(p1, p2, p3, p4, p5))
    # AVERAGE RUN 30 TIMES
    num = 30
    std = 1.5
    evaluated = np.zeros(num)
    for i in range(num):
        model.load_state_dict(torch.load('./results/Gauss/LeNet/LeNet-{}-{}-{}-{}-{}.pth'.format(p1, p2, p3, p4, p5)))
    ...
    if accu > best_accu:
        model.load_state_dict(torch.load('./results/Gauss/LeNet/LeNet-{}-{}-{}-{}-{}.pth'.format(p1, p2, p3, p4, p5)))
        torch.save(model.state_dict(), './results/Gauss/LeNet/LeNet-state-BEST.pth'.format(accu))
        ...
        for s in S:
            for n in range(N):
                # print(s)
                model.load_state_dict(torch.load('./results/Gauss/LeNet/LeNet-state-BEST.pth'.format(accu)))
        ...
        fig.savefig("./results/Gauss/LeNet/best_accu_{}.png".format(accu), dpi=320, bbox_inches='tight')
        results = np.vstack((S, np.array(A)))
        np.save("./results/Gauss/LeNet/best_accu_{}.npy".format(accu), results)
        best_accu = accu
...
def step_LeNet(n_iter):
    ...
    logger = JSONLogger(path="./results/Gauss/log/LeNet/logs1.json")
