...
def weights_mapping(file, mean, std, model, device='cuda'):
    ...
    the mean value. 
    """
    c_mean_smooth, c_std_smooth = calculate(file)
    ...
    with torch.no_grad():
        for theta in model.parameters():
            ...
            noise_sigma = torch.Tensor(np.log(1+np.square(yfit_std/yfit)))[noise_index] #MLE scalar 
            
            gassian_kernel = torch.distributions.Normal(0, noise_sigma)
...
def main():
    model1 = weights_mapping(0, 1, model=model, device='cpu')
