...
def weight_mapping_linear_only(f_name, model, noise_sigma, device='cuda'):
    """
    0911 for ASTE
    """
    c_mean_smooth = calculate_smoothed_cmean(f_name)
    ...
    gassian_kernel = torch.distributions.Normal(0.0, noise_sigma)
    with torch.no_grad():
        for theta in model.linear1.parameters():
            abstheta = torch.abs(theta) # 求参数的绝对值
            normalized_theta = abstheta / (torch.max(abstheta) + 1e-8) # 归一化
...
#         for theta in model.cls_linear.parameters():
#             abstheta = torch.abs(theta) # 求参数的绝对值
#             normalized_theta = abstheta / (torch.max(abstheta) + 1e-8) # 归一化
...
#         for theta in model.cls_linear1.parameters():
#             abstheta = torch.abs(theta) # 求参数的绝对值
#             normalized_theta = abstheta / (torch.max(abstheta) + 1e-8) # 归一化
